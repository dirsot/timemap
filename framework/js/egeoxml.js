/*******************************************************************************
 * *******************************************************************\ *
 * egeoxml.js by Mike Williams * * A Google Maps API Extension * * Renders the
 * contents of a My Maps (or similar) KML file * * Documentation:
 * http://econym.org.uk/gmap/egeoxml.htm * *
 * ********************************************************************** * This
 * Javascript is provided by Mike Williams * Community Church Javascript Team *
 * http://www.bisphamchurch.org.uk/ * http://econym.org.uk/gmap/ * * This work
 * is licenced under a Creative Commons Licence *
...
// Version 0.5 19 Apr 2007 - Accept an array of XML filenames, and add the
// {sortbyname} option
// Version 0.6 19 Apr 2007 - Sidebar entries for polylines, get directions and
// search nearby
...
// Version 1.2 25 Apr 2007 - If the description begins with "http://" make it
// into a link.
...
// Version 2.3 25 Nov 2007 - Clear variables used by .parse() so that it can be
// rerun
// Version 2.4 08 Dec 2007 - polylineoptions and polygonoptions
// Version 2.5 11 Dec 2007 - EGeoXml.value() trims leading and trailing
// whitespace
// Version 2.6 08 Feb 2008 - Trailing whitespace wasn't removed in the previous
// change
// Version 2.7 14 Oct 2008 - If there's no <description> try looking for <text>
// instead
...
function EGeoXml(myvar, doc, opts) {
	// store the parameters
	this.myvar = myvar;
	this.doc = doc;
	this.opts = opts || {};
	// infowindow styles
	this.titlestyle = this.opts.titlestyle
			|| 'style = "font-family: arial, sans-serif;font-size: medium;font-weight:bold;font-size: 100%;"';
	this.descstyle = this.opts.descstyle
			|| 'style = "font-family: arial, sans-serif;font-size: small;padding-bottom:.7em;"';
	this.directionstyle = this.opts.directionstyle
			|| 'style="font-family: arial, sans-serif;font-size: small;padding-left: 1px;padding-top: 1px;padding-right: 4px;"';
...
EGeoXml.prototype.createMarker = function(point, name, desc, style) {
	...
	var html = "<div style = 'width:" + this.iwwidth + "px'>" + "<h1 "
			+ this.titlestyle + ">" + name + "</h1>" + "<div " + this.descstyle
			+ ">" + desc + "</div>";

	if (this.opts.directions && false) {
		var html1 = html + '<div ' + this.directionstyle + '>'
				+ 'Get Directions: <a href="javascript:GEvent.trigger('
				+ this.myvar + '.lastmarker,\'click2\')">To Here</a> - '
				+ '<a href="javascript:GEvent.trigger(' + this.myvar
				+ '.lastmarker,\'click3\')">From Here</a><br>'
				+ '<a href="javascript:GEvent.trigger(' + this.myvar
				+ '.lastmarker,\'click4\')">Search nearby</a></div>';
		var html2 = html
				+ '<div '
				+ this.directionstyle
				+ '>'
				+ 'Get Directions: To here - '
				+ '<a href="javascript:GEvent.trigger('
				+ this.myvar
				+ '.lastmarker,\'click3\')">From Here</a><br>'
				...
				+ ',' + point.lng() + "(" + name + ")" + '"/>'
				+ '<br><a href="javascript:GEvent.trigger(' + this.myvar
				+ '.lastmarker,\'click\')">&#171; Back</a></div>';
		var html3 = html
				+ '<div '
				+ this.directionstyle
				+ '>'
		...
		var html4 = html
				...
				+ point.lat() + ',' + point.lng() + '"/>'
				// + '<input type="hidden" name="near" value="' + point.lat() +
				// ',' + point.lng() + "(" + name + ")" + '"/>';
				+ '<br><a href="javascript:GEvent.trigger(' + this.myvar
				+ '.lastmarker,\'click\')">&#171; Back</a></div>';
	...
	/*
	 * GEvent.addListener(m, "click", function() { eval(myvar+".lastmarker =
	 * m"); m.openInfoWindowHtml(html1 + "</div>",iwoptions); });
	 */
...
EGeoXml.prototype.createPolyline = function(points, color, width, opacity,
		name, desc) {
	...
	var html = "<div style='font-weight: bold; font-size: medium; margin-bottom: 0em;'>"
			+ name
			+ "</div>"
			+ "<div style='font-family: Arial, sans-serif;font-size: small;width:"
			+ this.iwwidth + "px'>" + desc + "</div>";

	if (this.opts.sidebarid) {
		var n = this.gpolylines.length - 1;
		var blob = '&nbsp;&nbsp;<span style=";border-left:' + width
				+ 'px solid ' + color + ';">&nbsp;</span> ';
...
EGeoXml.prototype.createPolygon = function(points, id, pId, begin, end, color,
	...
	if (this.opts.sidebarid) {
		...
		var blob = '<span style="background-color:' + fillcolor
...
EGeoXml.addSidebar = function(myvar, name, type, i, graphic) {
	if (type == "marker") {
		return '<a href="javascript:GEvent.trigger(' + myvar + '.gmarkers[' + i
				+ '],\'click\')">' + name + '</a><br>';
	}
	if (type == "polyline") {
		return '<div style="margin-top:6px;"><a href="javascript:GEvent.trigger('
				+ myvar
				+ '.gpolylines['
				+ i
				+ '],\'click\')">'
...
EGeoXml.prototype.processing = function(doc) {
	...
	for ( var i = 0; i < styles.length; i++) {
		...
		if (linestyles.length > 0) {
			...
			var color = EGeoXml.value(linestyles[0]
		...
		if (polystyles.length > 0) {
			...
			var color = EGeoXml.value(polystyles[0]
					.getElementsByTagName("color")[0]);
	...
	for ( var i = 0; i < placemarks.length; i++) {
		...
		var begin = EGeoXml.value(timeSpan.getElementsByTagName('begin')[0])
		var end = EGeoXml.value(timeSpan.getElementsByTagName('end')[0])
		var name = EGeoXml.value(placemarks[i].getElementsByTagName("name")[0]);
		var desc = EGeoXml.value(placemarks[i]
		...
		var style = EGeoXml.value(placemarks[i]
	...
	for ( var i = 0; i < grounds.length; i++) {
		...
		var north = parseFloat(GXml.value(grounds[i]
				.getElementsByTagName("north")[0]));
		var south = parseFloat(GXml.value(grounds[i]
				.getElementsByTagName("south")[0]));
		var east = parseFloat(GXml.value(grounds[i]
				.getElementsByTagName("east")[0]));
		var west = parseFloat(GXml.value(grounds[i]
				.getElementsByTagName("west")[0]));
		var sw = new google.maps.LatLng(south, west);
		var ne = new google.maps.LatLng(north, east);
		var ground = new GGroundOverlay(url, new google.maps.GLatLngBounds(sw,
				ne));
	...
	if (that.progress == 0) {
		...
		if (that.opts.sidebarid) {
			for ( var i = 0; i < that.side_bar_list.length; i++) {
				...
				that.side_bar_html += that.sidebarfn(that.myvar, bits[0],
		...
		if (that.opts.dropboxid) {
			for ( var i = 0; i < that.side_bar_list.length; i++) {
				var bits = that.side_bar_list[i].split("$$$", 4);
				if (bits[1] == "marker") {
					that.side_bar_html += that.dropboxfn(that.myvar, bits[0],
							bits[1], bits[2], bits[3]);
				}
			}
			document.getElementById(that.opts.dropboxid).innerHTML = '<select onChange="var I=this.value;if(I>-1){GEvent.trigger('
					+ that.myvar
					+ '.gmarkers[I],\'click\'); }">'
					+ '<option selected> - Select a location - </option>'
					+ that.side_bar_html + '</select>';
		}
