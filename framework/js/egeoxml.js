...
EGeoXml.prototype.createPolyline = function(points,color,width,opacity,name,desc) {
...
EGeoXml.prototype.createPolygon = function(points,color,width,opacity,fillcolor,fillopacity, name, desc) {
  var iwoptions = this.opts.iwoptions || {};
...
EGeoXml.prototype.processing = function(doc) {
    ...
    for (var i = 0; i < placemarks.length; i++) {
	  ...
	  polygons = multiGeometry2.getElementsByTagName("Polygon")[0]
	  coords=GXml.value(polygons.getElementsByTagName("coordinates")[0]);
	  path = coords.split(" ");
      ...
      if (path.length > 1) {
        ...
        if (linestring.length) {
          ...
          if (!!that.opts.createpolyline) {
            that.opts.createpolyline(points,color,width,opacity,name,desc);
          } else {
            that.createPolyline(points,color,width,opacity,name,desc);
          }
        }
        ...
        if (polygons.length) {
          ...
          if (!!that.opts.createpolygon) {
            that.opts.createpolygon(points,color,width,opacity,fillcolor,fillopacity,name,desc);
          } else {
            that.createPolygon(points,color,width,opacity,fillcolor,fillopacity,name,desc);
          }
        }
    ...
    for (var i = 0; i < grounds.length; i++) {
      ...
      var ground = new GGroundOverlay(url, new google.maps.GLatLngBounds(sw,ne));
