...
\item[Aproksymacja] \hfill \\
    Wygenerowany wynik jest przybliżeniem punktów wyjściowych, stworzona linia nie  musi przechodzić prze przez nie.

\item[Interpolacja] \hfill \\
    Wygenerowana funkcja iterpolacyjna przez wszystkie punkty określony w zbiorze wejściowym, jest jego dokładnym otworzeniem.Z tego powodu nadaje się lepiej do omawianegego celu.
    
\end{description}


Najprostrzym rodzajem jest interpolacja funkcji sklejonych stopnia pierwszego. Określa ona zbiór prostych odcinków łączących pary poszczególnych punktów. To dzięki niej przy użyciu 3 punktów możliwe jest stwrozenie trójkąta.
...
Jednym z rozwiązań powyższego problemu jest skorzystanie z krzywej Beziera. Rysunek \ref{fig:bezier} przedstawia prosty przykład pokazujący niezbędne informacje do jej stworzenia dla zbioru dwu-elementowego. Dodane zostały dwa punkty kontrolne, mają one za zadanie określić zachowanie ścieszki.
